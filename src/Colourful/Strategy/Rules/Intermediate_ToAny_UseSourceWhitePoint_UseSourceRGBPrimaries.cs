...
namespace Colourful.Strategy.Rules
{
    ...
    /// The intermediate node will use the source white point and RGB primaries.
    /// </summary>
    /// <typeparam name="TSource">Source color type</typeparam>
    /// <typeparam name="TIntermediate">Intermediate color type</typeparam>
    public class Intermediate_ToAny_UseSourceWhitePoint_UseSourceRGBPrimaries<TSource, TIntermediate> : IConversionRule
        ...
        where TIntermediate : struct
    {
        ...
        public bool TryApplyRule(in IConversionMetadata sourceNode, in IConversionMetadata targetNode, out IConversionMetadata[] replacementNodes, out object conversion)
        {
            ...
            var intermediateNode = new ConversionMetadata(CreateColorType<TIntermediate>(), CreateWhitePoint(sourceNode.GetWhitePoint()), CreateRGBPrimaries(sourceNode.GetRGBPrimaries()));
